...
全ワークフローで `runs-on` には `CI_RUNNER_LABEL` の参照式を使う:

```yaml
runs-on: ${{ vars.CI_RUNNER_LABEL || 'ubicloud-standard-2' }}
```

キュー飽和 / Mac Studio 障害時は `ci-queue-burst.yml` や `runner-health-check.yml` が `CI_RUNNER_LABEL` を一時的に空に切り替え、`ubicloud-standard-2` フォールバックに自動寄せする設計（cloudpanda の動的バースト機構）。**この変数を削除しないこと**。

旧来は cloudpanda PR [#5878](https://github.com/CloudPanda-Inc/cloudpanda/pull/5878) で導入された `runs-on: ${{ (vars.CI_BURST_ACTIVE != '' && github.run_attempt > 1) && 'ubicloud-standard-2' || vars.CI_RUNNER_LABEL || 'ubuntu-latest' }}` の並行ルーティング式が公式とされていたが、PR [#11297](https://github.com/CloudPanda-Inc/cloudpanda/pull/11297) で「`CI_RUNNER_LABEL` を一時削除する方式」に再設計済み。cloudpanda の `.github/workflows/` 全 55 ファイルは上記の単純式に統一されている。
